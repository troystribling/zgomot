...
A Note List is a set of notes that start playing simultaneously. The Note List may contain any number of notes and its duration is the duration of the longest note in the list. It is defined by

    nl(n1, n2, ..., nN)

where <tt>nN</tt> is the <tt>N'th</tt> note in the list.

Note Lists internally are arrays of notes and support the following array methods: <tt>reverse!, shift, pop, push, unshift</tt>
...
Progressions internally are arrays of notes and support the following array methods: <tt>reverse!, shift, pop, push, unshift</tt>
...
* <tt>t</tt>: Toggle the <tt>playing/paused</tt> status of selected streams. When <tt>t</tt> is entered use the up/down keys to traverse the list of streams and type return to select a stream. Multiple streams can be selected. Type <tt>t</tt> again to toggle the status of the selected streams.
* <tt>d</tt>: Delete selected streams. When <tt>d</tt> is entered use the up/down keys to traverse the list of streams and type return to select a stream. Multiple streams can be selected. Type <tt>d</tt> again to delete the selected streams.
