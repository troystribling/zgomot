...
A plock can also be passed to <tt>add_cc</tt> which is yielded the updated cc parameters. For example a CC which pauses a stream would look like,
...
* <tt>t</tt>: Toggle the <tt>playing/paused</tt> status of selected streams. When <tt>t</tt> is entered use the up/down keys to traverse the list of streams and type return to select a stream. Multiple streams can be selected in this manner. type <tt>t</tt> again to toggle the status of the selected streams.
* <tt>d</tt>: Delete selected streams. When <tt>d</tt> is entered use the up/down keys to traverse the list of streams and type return to select a stream. Multiple streams can be selected in this manner. type <tt>d</tt> again to delete the selected streams.
