...
A MIDI input device can be used to send messages to programs. Commands to list MIDI sources, add MIDI inputs devices and assign MIDI CC messages to progeram variables
are available.
...
A MIDI input device is added with
    ...
    add_input(device_name)

where <tt>device_name</tt> is the name of the MIDI source returned by the <tt>sources</tt> command.
...
Where name is the variable, ch is the MIDI CC identifier, a number between 0 and 255 and options are the following,
